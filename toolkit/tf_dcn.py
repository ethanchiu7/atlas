...
def cross_layer(x0, x, name):
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(name="weight", shape=[input_dim], initializer=tf.truncated_normal_initializer(stddev=0.01))
        b = tf.get_variable(name="bias", shape=[input_dim], initializer=tf.truncated_normal_initializer(stddev=0.01))
